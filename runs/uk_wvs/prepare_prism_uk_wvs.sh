...
PANEL_CONFIG="${PANEL_CONFIG:-configs/panel_config_uk.yaml}"
...
UK_REP_OUTPUT="${UK_REP_OUTPUT:-$DATA_DIR/uk_rep.jsonl}"
SOFT_OUTPUT="${SOFT_OUTPUT:-$DATA_DIR/uk_soft_panel.jsonl}"
HARD_OUTPUT="${HARD_OUTPUT:-$DATA_DIR/uk_hard_panel.jsonl}"
...
log "PRISM UK prep configuration:"
...
log "  UK_REP_OUTPUT=$UK_REP_OUTPUT"
...
UK_REP_CMD=(
  ...
  "${CONV_ARGS[@]}"
  --delta "$DELTA"
  --rater-normalization all
  --output "$UK_REP_OUTPUT"
)
if [[ ${#SYSTEM_PROMPT_ARGS[@]} -gt 0 ]]; then
  UK_REP_CMD+=("${SYSTEM_PROMPT_ARGS[@]}")
fi
run_step "Preparing PRISM UK representative dataset." "${UK_REP_CMD[@]}"
...
run_step "Preparing PRISM UK soft-panel dataset." "${SOFT_CMD[@]}"
...
run_step "Preparing PRISM UK hard-panel dataset." "${HARD_CMD[@]}"

log "Prepared PRISM UK datasets: $UK_REP_OUTPUT, $SOFT_OUTPUT, $HARD_OUTPUT"

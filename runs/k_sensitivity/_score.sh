...
log "k-sensitivity scoring / plotting configuration:"
...
log "  REFERENCE_MODEL=$REFERENCE_MODEL"
...
"$PYTHON" "$VIS_ROOT/model_vs_baseline_delta.py" \
  --preferences "$PREFERENCES_PATH" \
  --baseline-models "$REFERENCE_MODEL" \
  --output-dir "$PLOTS_DIR" \
  --plot-name "model_vs_reference_delta.png" \
  --csv-name "model_vs_reference_delta.csv"

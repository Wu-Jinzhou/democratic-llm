...
def fit_plackett_luce(
    ...
    for _ in range(max_iter):
        ...
        w = w / gm
        if np.max(np.abs(np.log(w) - np.log(w_old))) < tol:
            break
    return np.log(w), w
...
def bootstrap_plackett_luce(
    ...
    for s in range(num_samples):
        ...
        scores, _ = fit_plackett_luce(sample_rankings, models, max_iter=max_iter, tol=tol)
        samples[s, :] = scores
    return samples
...
def main() -> None:
    ...
    if args.method == "plackett-luce":
        pl_scores, pl_abilities = fit_plackett_luce(
            rankings, models, max_iter=args.pl_max_iter, tol=args.pl_tol
        )
        pl_results = []
        for i, m in enumerate(models):
            pl_results.append(
